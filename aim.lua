...
local Window = Fluent:CreateWindow({
    ...
    Center = true,
    IsDraggable = true,
    Scrollable = true,  -- Thêm tính năng cuộn
    ScrollBarThickness = 5,  -- Độ dày thanh cuộn
    ScrollBarColor = Color3.fromRGB(100, 100, 100)  -- Màu thanh cuộn
})

local function SetupScrolling(frame)
    local scrollingEnabled = false
    local scrollStartPos = nil
    local scrollStartOffset = nil
    
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            scrollingEnabled = true
            scrollStartPos = input.Position.Y
            scrollStartOffset = frame.CanvasPosition.Y
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if scrollingEnabled and input.UserInputType == Enum.UserInputType.MouseMovement then
            local delta = input.Position.Y - scrollStartPos
            frame.CanvasPosition = Vector2.new(0, scrollStartOffset - delta)
        end
    end)
end

-- Áp dụng scrolling cho tất cả các tab
for _, tab in pairs(Window.Tabs) do
    SetupScrolling(tab.Container)
end

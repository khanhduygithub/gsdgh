...
function Library:CreateToggleIconButton(config)
	local button = Instance.new("ImageButton")
	button.Name = config.Name or "ToggleIconButton"
	button.Size = UDim2.new(0, 40, 0, 40)
	button.BackgroundTransparency = 1
	button.Image = config.OffIcon or "rbxassetid://moon_icon_id" -- Mặc định là Mặt Trăng
	...
	button.MouseButton1Click:Connect(function()
		toggled = not toggled
		if toggled then
			button.Image = config.OnIcon or "rbxassetid://sun_icon_id"
			if config.OnToggle then
				config.OnToggle(true)
			end
		else
			button.Image = config.OffIcon or "rbxassetid://moon_icon_id"
			if config.OnToggle then
	...
	button.Parent = config.Parent or game.Players.LocalPlayer:WaitForChild("PlayerGui")

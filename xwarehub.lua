...
local requestsDisabled = true --getgenv and getgenv().DISABLE_KhanhDuy_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local KhanhDuyFolder = "KhanhDuy"
local ConfigurationFolder = KhanhDuyFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		KhanhDuyOpen = {Type = 'bind', Value = 'K', Name = 'KhanhDuy Keybind'},
		-- buildwarnings
		-- KhanhDuyprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(KhanhDuyFolder) then
				if isfile and isfile(KhanhDuyFolder..'/settings'..ConfigurationExtension) then
					file = readfile(KhanhDuyFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"KhanhDuyOpen":{"Value":"K","Type":"bind","Name":"KhanhDuy Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"KhanhDuy Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('KhanhDuy had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("KhanhDuy", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify KhanhDuy developers at sirius.menu/discord.")
				end
			end)
...
local KhanhDuy = useStudio and script.Parent:FindFirstChild('KhanhDuy') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if KhanhDuy:FindFirstChild('Build') and KhanhDuy.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('KhanhDuy | Build Mismatch')
		print('KhanhDuy may encounter issues as you are running an incompatible interface version ('.. ((KhanhDuy:FindFirstChild('Build') and KhanhDuy.Build.Value) or 'No Build') ..').\n\nThis version of KhanhDuy is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, KhanhDuy = KhanhDuy, useStudio and script.Parent:FindFirstChild('KhanhDuy') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

KhanhDuy.Enabled = false

if gethui then
	KhanhDuy.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(KhanhDuy)
	KhanhDuy.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	KhanhDuy.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == KhanhDuy.Name and Interface ~= KhanhDuy then
			Interface.Enabled = false
			Interface.Name = "KhanhDuy-Old"
		end
	end
elseif not useStudio then
...
if KhanhDuy.AbsoluteSize.X < minSize.X and KhanhDuy.AbsoluteSize.Y < minSize.Y then
...
local Main = KhanhDuy.Main
local MPrompt = KhanhDuy:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = KhanhDuy:FindFirstChild('Drag')
...
KhanhDuy.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/KhanhDuy/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = KhanhDuy.Notifications

local SelectedTheme = KhanhDuyLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = KhanhDuyLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	KhanhDuy.Main.BackgroundColor3 = SelectedTheme.Background
	KhanhDuy.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	KhanhDuy.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	KhanhDuy.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	KhanhDuy.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	KhanhDuy.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	KhanhDuy.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		KhanhDuy.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		KhanhDuy.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(KhanhDuy:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("KhanhDuy | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("KhanhDuy | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('KhanhDuy had an issue decoding the configuration file, please try delete the file and reopen KhanhDuy.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(KhanhDuyLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("KhanhDuy | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--KhanhDuyLibrary:Notify({Title = "KhanhDuy Flags", Content = "KhanhDuy was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(KhanhDuyLibrary.Flags) do
...
function KhanhDuyLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('KhanhDuy | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			KhanhDuyLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Khanh Duy'.", Duration = 7, Image = 4400697855})
		else
			KhanhDuyLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.KhanhDuyOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(KhanhDuyFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('KhanhDuy Settings', 0, true)

	if TabList['KhanhDuy Settings'] then
		TabList['KhanhDuy Settings'].LayoutOrder = 1000
	end

	if Elements['KhanhDuy Settings'] then
		Elements['KhanhDuy Settings'].LayoutOrder = 1000
	end
...
function KhanhDuyLibrary:CreateWindow(Settings)
	if KhanhDuy:FindFirstChild('Loading') then
		if getgenv and not getgenv().KhanhDuyCached then
			KhanhDuy.Enabled = true
			KhanhDuy.Loading.Visible = true

			task.wait(1.4)
			KhanhDuy.Loading.Visible = false
		end
	end

	if getgenv then getgenv().KhanhDuyCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				KhanhDuyLibrary:Notify({Title = 'Build Mismatch', Content = 'KhanhDuy may encounter issues as you are running an incompatible interface version ('.. ((KhanhDuy:FindFirstChild('Build') and KhanhDuy.Build.Value) or 'No Build') ..').\n\nThis version of KhanhDuy is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(KhanhDuyFolder) then
		makefolder(KhanhDuyFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "KhanhDuy"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "KhanhDuy Interface Suite" then
		LoadingFrame.Version.Text = "KhanhDuy UI"
	end
	...
	if not Settings.DisableKhanhDuyPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				KhanhDuyLibrary:Notify({
					Title = "KhanhDuy Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(KhanhDuyFolder.."/Discord Invites") then
			makefolder(KhanhDuyFolder.."/Discord Invites")
		end

		if isfile and not isfile(KhanhDuyFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(KhanhDuyFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"KhanhDuy RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(KhanhDuyFolder.."/Key System") then
			makefolder(KhanhDuyFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("KhanhDuy | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
				end
			end
		end
		...
		if isfile and isfile(KhanhDuyFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(KhanhDuyFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(KhanhDuyFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						KhanhDuyLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("KhanhDuy | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= KhanhDuyLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					KhanhDuyLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			KhanhDuy.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("KhanhDuy | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					KhanhDuyLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("KhanhDuy | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					KhanhDuyLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("KhanhDuy | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					KhanhDuyLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= KhanhDuyLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("KhanhDuy | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						KhanhDuyLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("KhanhDuy | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with KhanhDuy specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					KhanhDuyLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			KhanhDuyLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			KhanhDuyLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('KhanhDuy had an issue creating settings.') end
...
function KhanhDuyLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function KhanhDuyLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function KhanhDuyLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['KhanhDuy Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.KhanhDuyOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function KhanhDuyLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				KhanhDuyLibrary:Notify({Title = "KhanhDuy Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			KhanhDuyLibrary:Notify({Title = "KhanhDuy Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('KhanhDuy Configurations Error | '..tostring(result))
			KhanhDuyLibrary:Notify({Title = "KhanhDuy Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = KhanhDuyLibrary:CreateWindow({
		Name = "KhanhDuy Example Window",
		LoadingTitle = "KhanhDuy Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using KhanhDuy may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like KhanhDuy to get the key from
	...
	--KhanhDuyLibrary:Notify({Title = "KhanhDuy Interface", Content = "Welcome to KhanhDuy. These - are the brand new notification design for KhanhDuy, with custom sizing and KhanhDuy calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(KhanhDuyLibrary.Theme) do
...
task.delay(4, function()
	KhanhDuyLibrary.LoadConfiguration()
